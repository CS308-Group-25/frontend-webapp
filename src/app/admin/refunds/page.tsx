...
import { fetchAdminRefunds, updateRefundStatus } from '@/features/admin/refunds/api';
import { AdminRefundRequest, RefundStatus } from '@/features/admin/refunds/types';
...
export default function AdminRefundsPage() {
  ...
  const counts = useMemo(() => ({
    ...
    approved_waiting_return:refunds.filter((r) => r.status === 'approved_waiting_return').length,
    returned_received:      refunds.filter((r) => r.status === 'returned_received').length,
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">İade Talepleri</h1>
              <p className="mt-1 text-sm text-slate-500">
      ...
      <div className="mb-6 grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          { label: 'Talep',       count: counts.requested,                                              color: 'bg-amber-50 text-amber-700' },
          { label: 'İşlemde',     count: counts.approved_waiting_return + counts.returned_received,     color: 'bg-blue-50 text-blue-700' },
          { label: 'Tamamlandı',  count: counts.refunded,                                               color: 'bg-green-50 text-green-700' },
          { label: 'Reddedildi',  count: counts.rejected,                                               color: 'bg-red-50 text-red-700' },
        ].map((card) => (
          <div key={card.label} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 flex flex-col">
      ...
      {loading ? (
        <div className="flex min-h-[400px] items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
          <div className="flex flex-col items-center gap-3">
            <div className="h-8 w-8 animate-spin rounded-full border-4 border-indigo-600 border-t-transparent" />
            <p className="text-sm font-medium text-slate-500">İade talepleri yükleniyor...</p>
          </div>
        </div>
      ) : fetchError ? (
        <div className="flex min-h-[400px] flex-col items-center justify-center rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-red-100">
          ...
          <p className="text-lg font-bold text-red-600">İade talepleri yüklenemedi.</p>
          <p className="mt-1 text-sm text-red-400">Lütfen bağlantınızı kontrol edip tekrar deneyin.</p>
          <button onClick={loadRefunds} className="mt-4 text-sm font-medium text-indigo-600 hover:underline">
            Tekrar Dene
          </button>
        </div>
      ) : (
        <RefundRequestsTable refunds={filteredRefunds} onUpdateStatus={handleUpdateStatus} />
      )}
    </div>
  );
}
